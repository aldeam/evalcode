...
/**
 * This file contains the functions for evalcode_plugin abstract class
 *
 *
...
/**
 * Abstract class for evalcode_plugin (submission/feedback).
 *
 * @package   mod_evalcode
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class evalcode_plugin {

    /** @var evalcode $evalcodeframework the evalcodeframework record that contains the global
     *              settings for this evalcode instance
     */
    protected $evalcodeframework;
    /** @var string $type evalcodeframework plugin type */
    private $type = '';
     ...
     *
     * @param evalcode $evalcodeframework
     * @param string $type
     */
    public final function __construct(evalcode $evalcodeframework, $type) {
     ...
     * form for display in the settings page for the evalcodeframework.
    ...
    /**
     * The evalcodeframework subtype is responsible for saving it's own settings as the database table for the
     ...
     *
     * @return string - either 'evalsubmission' or 'feedback'
     */
     ...
     *
     * @param stdClass $submissionorgrade - eval_submission or evalcode_grade.
    ...
    /**
     * Get any additional fields for the submission/grading form for this evalcodeframework.
     *
    ...
    public final function has_admin_settings() {
        ...
        $pluginroot = $CFG->dirroot . '/mod/evalcode/' . substr($this->get_subtype(), strlen('evalcode')) . '/' . $this->get_type();
    ...
    public final function set_config($name, $value) {
        ...
        $dbparams = array('evalcodeframework'=>$this->evalcodeframework->get_instance()->id,
                          'subtype'=>$this->get_subtype(),
                          'plugin'=>$this->get_type(),
                          'name'=>$name);
        $current = $DB->get_record('evalcode_plugin_config', $dbparams, '*', IGNORE_MISSING);

        if ($current) {
            $current->value = $value;
            return $DB->update_record('evalcode_plugin_config', $current);
        } else {
            $setting = new stdClass();
            $setting->evalcodeframework = $this->evalcodeframework->get_instance()->id;
            $setting->subtype = $this->get_subtype();
            $setting->plugin = $this->get_type();
            $setting->name = $name;
            $setting->value = $value;

            return $DB->insert_record('evalcode_plugin_config', $setting) > 0;
        }
    }
    ...
    public final function get_config($setting = null) {
        ...
        if ($setting) {
            if (!$this->evalcodeframework->has_instance()) {
                return false;
            }
            $evalcodeframework = $this->evalcodeframework->get_instance();
            if ($evalcodeframework) {
                $dbparams = array('evalcodeframework'=>$evalcodeframework->id,
                                  'subtype'=>$this->get_subtype(),
                                  'plugin'=>$this->get_type(),
                                  'name'=>$setting);
                $result = $DB->get_record('evalcode_plugin_config', $dbparams, '*', IGNORE_MISSING);
                if ($result) {
                    return $result->value;
                }
            }
        ...
        $dbparams = array('evalcodeframework'=>$this->evalcodeframework->get_instance()->id,
                          'subtype'=>$this->get_subtype(),
                           'plugin'=>$this->get_type());
        $results = $DB->get_records('evalcode_plugin_config', $dbparams);
     ...
     * and version.
     *
     * @param string $type The old evalcodeframework subtype
     * @param int $version The old evalcodeframework version
    ...
    /**
     * Upgrade the settings from the old evalcodeframework to the new one
     *
     * @param context $oldcontext The context for the old evalcodeframework module
    ...
    public function upgrade_settings(context $oldcontext, stdClass $oldevalcodeframework, & $log) {
        ...
        $log .= ' ' . get_string('upgradenotimplemented', 'mod_evalcode', $params);
        return false;
    }
     ...
     * @param stdClass $submissionorgrade eval_submission or evalcode_grade The new submission or grade
     * @param string $log Record upgrade messages in the log
    ...
    public function upgrade(context $oldcontext,
                            ...
                            & $log) {
        $params = array('type'=>$this->type, 'subtype'=>$this->get_subtype());
        $log = $log . ' ' . get_string('upgradenotimplemented', 'mod_evalcode', $params);
        return false;
    }
    ...
    /**
     * The evalcodeframework has been deleted - remove the plugin specific data
     *
     * @return bool
     */
    ...
    /**
     * Is this evalcodeframework plugin empty? (ie no submission or feedback)
     * @param stdClass $submissionorgrade eval_submission or evalcode_grade
     * @return bool
     */
    ...
    public function get_file_info($browser, $filearea, $itemid, $filepath, $filename) {
        ...
        if ($this->get_subtype() == 'evalsubmission') {
            if ($itemid) {
                $record = $DB->get_record('eval_submission', array('id'=>$itemid), 'userid', IGNORE_MISSING);
                if (!$record) {
                    return null;
                }
                if (!$this->evalcodeframework->can_view_submission($record->userid)) {
                    return null;
                }
            }
        } else {
        ...
        if (!($storedfile = $fs->get_file($this->evalcodeframework->get_context()->id,
    ...
    /**
     * This allows a plugin to render a page in the context of the evalcodeframework
     *
     * If the plugin creates a link to the evalcodeframework view.php page with
     * The following required parameters:
     *      id=coursemoduleid
     *      plugin=type
     *      pluginsubtype=evalfeedback|evalsubmission
     ...
     * right below the activity's "intro" section on the main evalcodeframework page.
     *
     * @return string
     */
     ...
     * enabled/disabled per evalcodeframework instance.
     *
     * @return bool
     */
