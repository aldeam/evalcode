...
class evalsubmission_file_events_testcase extends advanced_testcase {

    /** @var stdClass $user A user to submit an evalcodeframework. */
    protected $user;

    /** @var stdClass $course New course created to hold the evalcodeframework activity. */
    protected $course;

    /** @var stdClass $cm A context module object. */
    protected $cm;

    /** @var stdClass $context Context of the evalcodeframework activity. */
    protected $context;

    /** @var stdClass $evalcode The evalcodeframework object. */
    protected $evalcode;

    /** @var stdClass $files Files that are being submitted for the evalcodeframework. */
    protected $files;
    ...
    /**
     * Setup all the various parts of an evalcodeframework activity including creating a file submission.
     */
    protected function setUp() {
        $this->user = $this->getDataGenerator()->create_user();
        $this->course = $this->getDataGenerator()->create_course();
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_evalcode');
        $params['course'] = $this->course->id;
        $instance = $generator->create_instance($params);
        $this->cm = get_coursemodule_from_instance('evalcode', $instance->id);
        $this->context = context_module::instance($this->cm->id);
        $this->evalcode = new testable_evalcode($this->context, $this->cm, $this->course);

        $this->setUser($this->user->id);
        $this->submission = $this->evalcode->get_user_submission($this->user->id, true);

        $fs = get_file_storage();
        $dummy = (object) array(
            'contextid' => $this->context->id,
            'component' => 'evalsubmission_file',
            'filearea' => EVALSUBMISSION_FILE_FILEAREA,
            'itemid' => $this->submission->id,
            'filepath' => '/',
            'filename' => 'myevalcodemnent.pdf'
        );
        ...
        $dummy = (object) array(
            ...
            'itemid' => $this->submission->id,
            'filepath' => '/',
            'filename' => 'myevalcodemnent.png'
        );
        $this->fi2 = $fs->create_file_from_string($dummy, 'Content of ' . $dummy->filename);
        $this->files = $fs->get_area_files($this->context->id, 'evalsubmission_file', EVALSUBMISSION_FILE_FILEAREA,
    ...
    public function test_assessable_uploaded() {
        ...
        $data = new stdClass();
        $plugin = $this->evalcode->get_submission_plugin_by_type('file');
        ...
        $event = reset($events);
        $this->assertInstanceOf('\evalsubmission_file\event\assessable_uploaded', $event);
    ...
    public function test_submission_created() {
        ...
        $event = $events[1];
        $this->assertInstanceOf('\evalsubmission_file\event\submission_created', $event);
        $this->assertEquals($this->context->id, $event->contextid);
    ...
    public function test_submission_updated() {
        ...
        // We want to test the last event fired.
        $event = $events[3];
        $this->assertInstanceOf('\evalsubmission_file\event\submission_updated', $event);
        $this->assertEquals($this->context->id, $event->contextid);
        $this->assertEquals($this->course->id, $event->courseid);
        $this->assertEquals($this->submission->id, $event->other['submissionid']);
        $this->assertEquals($this->submission->attemptnumber, $event->other['submissionattempt']);
