 ...
 * @return bool
 */
function evalsubmission_comments_comment_validate(stdClass $options) {
    global $USER, $CFG, $DB;
    ...
    if (!$submission = $DB->get_record('eval_submission', array('id'=>$options->itemid))) {
    ...
    if ($evalcodeframework->get_instance()->id != $submission->evalcodeframework) {
        throw new comment_exception('invalidcontext');
    }
    $canview = false;
    if ($submission->userid) {
        $canview = $evalcodeframework->can_view_submission($submission->userid);
    } else {
        $canview = $evalcodeframework->can_view_group_submission($submission->groupid);
    }
    if (!$canview) {
 ...
 * @return array
 */
function evalsubmission_comments_comment_permissions(stdClass $options) {
    global $USER, $CFG, $DB;
    ...
    if ($evalcodeframework->get_instance()->teamsubmission &&
        !$evalcodeframework->can_view_group_submission($submission->groupid)) {
        return array('post' => false, 'view' => false);
    }

    if (!$evalcodeframework->get_instance()->teamsubmission &&
        !$evalcodeframework->can_view_submission($submission->userid)) {
        return array('post' => false, 'view' => false);
    }
 ...
 */
function evalsubmission_comments_comment_display($comments, $options) {
    global $CFG, $DB, $USER;
    ...
    if ($evalcodeframework->is_blind_marking() && !empty($comments)) {
        // Blind marking is being used, may need to map unique anonymous ids to the comments.
        $usermappings = array();
        $hiddenuserstr = trim(get_string('hiddenuser', 'evalcode'));
        ...
        foreach ($comments as $comment) {
            ...
            if (empty($usermappings[$comment->userid])) {
                ...
                $anonid = $evalcodeframework->get_uniqueid_for_user($comment->userid);
                ...
                $usermappings[$comment->userid]->avatar = $evalcodeframework->get_renderer()->user_picture($commenter,
 ...
 */
function evalsubmission_comments_comment_add(stdClass $comment, stdClass $param) {

    global $DB;
    if ($comment->commentarea == 'submission_comments_upgrade') {
        $submissionid = $comment->itemid;
        $submission = $DB->get_record('eval_submission', array('id' => $submissionid));
