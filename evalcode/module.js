...
M.mod_evalcode.init_tree = function(Y, expand_all, htmlid) {
...
M.mod_evalcode.init_grading_table = function(Y) {
        ...
        if (batchform) {
            batchform.on('submit', function(e) {
                ...
                if (selectedusers.length == 0) {
                    alert(M.util.get_string('nousersselected', 'evalcode'));
                    e.preventDefault();
                } else {
...
M.mod_evalcode.init_grading_options = function(Y) {
    Y.use('node', function(Y) {
...
M.mod_evalcode.init_grade_change = function(Y) {
    ...
    if (gradenode) {
        ...
        gradenode.on('change', function() {
            if (gradenode.get('value') != originalvalue) {
                alert(M.util.get_string('changegradewarning', 'mod_evalcode'));
            }
        });
    }
};

M.mod_evalcode.init_plugin_summary = function(Y, subtype, type, submissionid) {
