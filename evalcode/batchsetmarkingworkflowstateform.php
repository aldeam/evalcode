...
require_once($CFG->dirroot . '/mod/evalcode/feedback/file/locallib.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_evalcode_batch_set_marking_workflow_state_form extends moodleform {
    /**
    ...
    public function definition() {
        ...
        $formheader = get_string('batchsetmarkingworkflowstateforusers', 'evalcode', $params['userscount']);

        $mform->addElement('header', 'general', $formheader);
        $mform->addElement('static', 'userslist', get_string('selectedusers', 'evalcode'), $params['usershtml']);

        $options = $params['markingworkflowstates'];
        $mform->addElement('select', 'markingworkflowstate', get_string('markingworkflowstate', 'evalcode'), $options);
        ...
        $mform->disabledIf('sendstudentnotifications', 'markingworkflowstate', 'neq', EVALCODE_MARKING_WORKFLOW_STATE_RELEASED);
    ...
    public function validation($data, $files) {
        ...
        if (!empty($data['sendstudentnotifications']) && $data['markingworkflowstate'] != EVALCODE_MARKING_WORKFLOW_STATE_RELEASED) {
            $errors['sendstudentnotifications'] = get_string('studentnotificationworkflowstateerror', 'evalcode');
        }
