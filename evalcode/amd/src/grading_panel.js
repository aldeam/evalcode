 ...
 *
 * @module     mod_evalcode/grading_panel
...
define(['jquery', 'core/notification', 'core/templates', 'core/fragment',
        'core/ajax', 'core/str', 'mod_evalcode/grading_form_change_checker',
        'mod_evalcode/grading_events'],
    ...
    GradingPanel.prototype._submitForm = function(event, nextUserId) {
        ...
        var evalcodeframeworkid = this._region.attr('data-evalcodeframeworkid');

        // Now we can continue...
        ajax.call([{
            methodname: 'mod_evalcode_submit_grading_form',
            args: {evalcodeframeworkid: evalcodeframeworkid, userid: this._lastUserId, jsonformdata: JSON.stringify(data)},
    ...
    GradingPanel.prototype._handleFormSubmissionResponse = function(formdata, nextUserId, response) {
        ...
        if (response.length) {
            ...
            str.get_strings([
                ...
                { key: 'gradechangessaveddetail', component: 'mod_evalcode' },
            ]).done(function(strs) {
    ...
    GradingPanel.prototype._chooseAttempt = function(e) {
        ...
        str.get_strings([
            { key: 'viewadifferentattempt', component: 'mod_evalcode' },
    ...
    GradingPanel.prototype._addPopoutButtons = function(selector) {
        ...
        templates.render('mod_evalcode/popout_button', {}).done(function(html) {
    ...
    GradingPanel.prototype._refreshGradingPanel = function(event, userid, submissiondata, attemptnumber) {
        ...
        window.M.util.js_pending('mod-evalcode-loading-user');
        // First insert the loading template.
        templates.render('mod_evalcode/loading', {}).done(function(html, js) {
            // Update the page.
            this._niceReplaceNodeContents(this._region, html, js).done(function() {
                if (userid > 0) {
                    ...
                    fragment.loadFragment('mod_evalcode', 'gradingpanel', contextid, params).done(function(html, js) {
                        ...
                        .done(function() {
                            ...
                            window.M.util.js_complete('mod-evalcode-loading-user');
                        }.bind(this))
                        .fail(notification.exception);
                    }.bind(this)).fail(notification.exception);
                } else {
