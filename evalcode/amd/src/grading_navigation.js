 ...
 *
 * @module     mod_evalcode/grading_navigation
...
define(['jquery', 'core/notification', 'core/str', 'core/form-autocomplete',
        'core/ajax', 'mod_evalcode/grading_form_change_checker'],
    ...
    var GradingNavigation = function(selector) {
        ...
        str.get_string('changeuser', 'mod_evalcode').done(function(s) {
                autocomplete.enhance('[data-action=change-user]', false, 'mod_evalcode/participant_selector', s);
            }.bind(this)
    ...
    /**
     * Load the list of all users for this evalcodeframework.
     *
     * @private
    ...
    GradingNavigation.prototype._loadAllUsers = function() {
        ...
        var evalcodeframeworkid = select.attr('data-evalcodeframeworkid');
        var groupid = select.attr('data-groupid');

        ajax.call([{
            methodname: 'mod_evalcode_list_participants',
            args: { evalcodeid: evalcodeframeworkid, groupid: groupid, filter: '', onlyids: true },
    ...
    GradingNavigation.prototype._checkClickOutsideConfigureFilters = function(event) {
        ...
        if (!configPanel.is(event.target) && configPanel.has(event.target).length === 0) {
            ...
            $(document).unbind('click.mod_evalcode_grading_navigation');
        }
    };
    ...
    GradingNavigation.prototype._filterChanged = function(event) {
        ...
        if (filterlist.length) {
            ...
            str.get_string('nofilters', 'mod_evalcode').done(function(s) {
    ...
    GradingNavigation.prototype._selectNoUser = function() {
        ...
        if (checker.checkFormForChanges('[data-region="grade-panel"] .gradeform')) {
            ...
            str.get_strings([
                { key: 'unsavedchanges', component: 'mod_evalcode' },
                { key: 'unsavedchangesquestion', component: 'mod_evalcode' },
                { key: 'saveandcontinue', component: 'mod_evalcode' },
    ...
    GradingNavigation.prototype._toggleExpandFilters = function(event) {
        ...
        if (expanded) {
            ...
            $(document).on('click.mod_evalcode_grading_navigation', this._checkClickOutsideConfigureFilters.bind(this));
        }
    };
    ...
    GradingNavigation.prototype._refreshCount = function() {
        ...
        if (isNaN(userid) || userid <= 0) {
            ...
            str.get_string('xofy', 'mod_evalcode', param).done(function(s) {
