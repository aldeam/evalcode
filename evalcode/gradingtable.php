...
/**
 * Extends table_sql to provide a table of evalcodeframework submissions
 *
 * @package   mod_evalcode
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class evalcode_grading_table extends table_sql implements renderable {
    /** @var evalcode $evalcodeframework */
    private $evalcodeframework = null;
    /** @var int $perpage */
    private $perpage = 10;
    ...
    /**
     * overridden constructor keeps a reference to the evalcodeframework class that is displaying this table
     *
     ...
     */
    public function __construct(evalcode $evalcodeframework,
                                $perpage,
                                $filter,
                                $rowoffset,
                                $quickgrading,
        ...
        parent::__construct('mod_evalcode_grading');
        $this->is_persistent(true);
        $this->evalcodeframework = $evalcodeframework;

        // Check permissions up front.
        $this->hasgrantextension = has_capability('mod/evalcode:grantextension',
                                                  $this->evalcodeframework->get_context());
        $this->hasgrade = $this->evalcodeframework->can_grade();

        // Check if we have the elevated view capablities to see the blind details.
        $this->hasviewblind = has_capability('mod/evalcode:viewblinddetails',
                $this->evalcodeframework->get_context());
        ...
        $this->output = $PAGE->get_renderer('mod_evalcode');

        $urlparams = array('action'=>'grading', 'id'=>$evalcodeframework->get_course_module()->id);
        $url = new moodle_url($CFG->wwwroot . '/mod/evalcode/view.php', $urlparams);
        ...
        $users = array_keys( $evalcodeframework->list_participants($currentgroup, true));
        if (count($users) == 0) {
        ...
        $params = array();
        $params['evalcodeframeworkid1'] = (int)$this->evalcodeframework->get_instance()->id;
        $params['evalcodeframeworkid2'] = (int)$this->evalcodeframework->get_instance()->id;
        $params['evalcodeframeworkid3'] = (int)$this->evalcodeframework->get_instance()->id;

        $extrauserfields = get_extra_user_fields($this->evalcodeframework->get_context());
        ...
        // For group submissions we don't immediately create an entry in the eval_submission table for each user,
        // instead the userid is set to 0. In this case we use a different query to retrieve the grade for the user.
        if ($this->evalcodeframework->get_instance()->teamsubmission) {
            $params['evalcodeframeworkid4'] = (int) $this->evalcodeframework->get_instance()->id;
            $grademaxattempt = 'SELECT mxg.userid, MAX(mxg.attemptnumber) AS maxattempt
                                  FROM {evalcode_grades} mxg
                                 WHERE mxg.evalcodeframework = :evalcodeframeworkid4
                              GROUP BY mxg.userid';
        ...
        $from .= 'LEFT JOIN {evalcode_user_flags} uf
                         ON u.id = uf.userid
                        AND uf.evalcodeframework = :evalcodeframeworkid3 ';

        if (!empty($this->evalcodeframework->get_instance()->blindmarking)) {
            $from .= 'LEFT JOIN {evalcode_user_mapping} um
                             ON u.id = um.userid
                            AND um.evalcodeframework = :evalcodeframeworkid4 ';
            $params['evalcodeframeworkid4'] = (int)$this->evalcodeframework->get_instance()->id;
        ...
        if ($this->evalcodeframework->is_any_submission_plugin_enabled()) {
            if ($filter == EVALCODE_FILTER_SUBMITTED) {
                $where .= ' AND (s.timemodified IS NOT NULL AND
                                 s.status = :submitted) ';
                $params['submitted'] = EVAL_SUBMISSION_STATUS_SUBMITTED;

            } else if ($filter == EVALCODE_FILTER_NOT_SUBMITTED) {
                $where .= ' AND (s.timemodified IS NULL OR s.status != :submitted) ';
                $params['submitted'] = EVAL_SUBMISSION_STATUS_SUBMITTED;
            } else if ($filter == EVALCODE_FILTER_REQUIRE_GRADING) {
                $where .= ' AND (s.timemodified IS NOT NULL AND
                                 s.status = :submitted AND
                                 (s.timemodified >= g.timemodified OR g.timemodified IS NULL OR g.grade IS NULL))';
                $params['submitted'] = EVAL_SUBMISSION_STATUS_SUBMITTED;

            } else if (strpos($filter, EVALCODE_FILTER_SINGLE_USER) === 0) {
        ...
        if ($this->evalcodeframework->get_instance()->markingworkflow &&
            $this->evalcodeframework->get_instance()->markingallocation) {
            if (has_capability('mod/evalcode:manageallocations', $this->evalcodeframework->get_context())) {
                // Check to see if marker filter is set.
                $markerfilter = (int)get_user_preferences('evalcode_markerfilter', '');
                if (!empty($markerfilter)) {
                    if ($markerfilter == EVALCODE_MARKER_FILTER_NO_MARKER) {
        ...
        if ($this->evalcodeframework->get_instance()->markingworkflow) {
            $workflowstates = $this->evalcodeframework->get_marking_workflow_states_for_current_user();
            if (!empty($workflowstates)) {
                $workflowfilter = get_user_preferences('evalcode_workflowfilter', '');
                if ($workflowfilter == EVALCODE_MARKING_WORKFLOW_STATE_NOTMARKED) {
                    $where .= ' AND (uf.workflowstate = :workflowstate OR uf.workflowstate IS NULL OR '.
                        $DB->sql_isempty('evalcode_user_flags', 'workflowstate', true, true).')';
        ...
        // User picture.
        if ($this->hasviewblind || !$this->evalcodeframework->is_blind_marking()) {
            if (!$this->is_downloading()) {
                $columns[] = 'picture';
                $headers[] = get_string('pictureofuser');
            } else {
                $columns[] = 'recordid';
                $headers[] = get_string('recordid', 'evalcode');
            }
            ...
            if ($this->evalcodeframework->is_blind_marking()) {
                if (!$this->is_downloading()) {
                    $columns[] = 'recordid';
                    $headers[] = get_string('recordid', 'evalcode');
                }
        ...
        // Submission status.
        $columns[] = 'status';
        $headers[] = get_string('status', 'evalcode');

        // Team submission columns.
        if ($evalcodeframework->get_instance()->teamsubmission) {
            $columns[] = 'team';
            $headers[] = get_string('submissionteam', 'evalcode');
        }
        // Allocated marker.
        if ($this->evalcodeframework->get_instance()->markingworkflow &&
            $this->evalcodeframework->get_instance()->markingallocation &&
            has_capability('mod/evalcode:manageallocations', $this->evalcodeframework->get_context())) {
            // Add a column for the allocated marker.
            $columns[] = 'allocatedmarker';
            $headers[] = get_string('marker', 'evalcode');
        }
        // Grade.
        $columns[] = 'grade';
        $headers[] = get_string('grade');
        if ($this->is_downloading()) {
            if ($this->evalcodeframework->get_instance()->grade >= 0) {
                $columns[] = 'grademax';
                $headers[] = get_string('maxgrade', 'evalcode');
            } else {
                // This is a custom scale.
                $columns[] = 'scale';
                $headers[] = get_string('scale', 'evalcode');
            }

            if ($this->evalcodeframework->get_instance()->markingworkflow) {
                // Add a column for the marking workflow state.
                $columns[] = 'workflowstate';
                $headers[] = get_string('markingworkflowstate', 'evalcode');
            }
            // Add a column for the list of valid marking workflow states.
            $columns[] = 'gradecanbechanged';
            $headers[] = get_string('gradecanbechanged', 'evalcode');
        }
        if (!$this->is_downloading() && $this->hasgrade) {
        ...
        // Submission plugins.
        if ($evalcodeframework->is_any_submission_plugin_enabled()) {
            $columns[] = 'timesubmitted';
            $headers[] = get_string('lastmodifiedsubmission', 'evalcode');

            foreach ($this->evalcodeframework->get_submission_plugins() as $plugin) {
                if ($this->is_downloading()) {
                    if ($plugin->is_visible() && $plugin->is_enabled()) {
        ...
        // Time marked.
        $columns[] = 'timemarked';
        $headers[] = get_string('lastmodifiedgrade', 'evalcode');

        // Feedback plugins.
        foreach ($this->evalcodeframework->get_feedback_plugins() as $plugin) {
            if ($this->is_downloading()) {
                if ($plugin->is_visible() && $plugin->is_enabled()) {
                    foreach ($plugin->get_editor_fields() as $field => $description) {
                        $index = 'plugin' . count($this->plugincache);
        ...
        $this->gradinginfo = grade_get_grades($this->evalcodeframework->get_course()->id,
                                              'mod',
                                              'evalcode',
                                              $this->evalcodeframework->get_instance()->id,
                                              $users);
    ...
    /**
     * Add a column with an ID that uniquely identifies this user in this evalcodeframework.
     *
     * @param stdClass $row
     * @return string
     */
    public function col_recordid(stdClass $row) {
        if (empty($row->recordid)) {
            $row->recordid = $this->evalcodeframework->get_uniqueid_for_user($row->userid);
        }
        return get_string('hiddenuser', 'evalcode') . $row->recordid;
    }
    ...
    public function col_workflowstatus(stdClass $row) {
        ...
        $gradingdisabled = $this->evalcodeframework->grading_disabled($row->id);
        // The function in the evalcodeframework keeps a static cache of this list of states.
        $workflowstates = $this->evalcodeframework->get_marking_workflow_states_for_current_user();
        $workflowstate = $row->workflowstate;
        if (empty($workflowstate)) {
            $workflowstate = EVALCODE_MARKING_WORKFLOW_STATE_NOTMARKED;
        }
        if ($this->quickgrading && !$gradingdisabled) {
            ...
            $o .= $this->output->container(get_string('markingworkflowstate' . $workflowstate, 'evalcode'), $workflowstate);
        }
        return $o;
    }
    ...
    public function col_workflowstate($row) {
        ...
        if (empty($state)) {
            $state = EVALCODE_MARKING_WORKFLOW_STATE_NOTMARKED;
        }

        return get_string('markingworkflowstate' . $state, 'evalcode');
    }
    ...
    public function col_allocatedmarker(stdClass $row) {
        ...
        if ($markers === null) {
            ...
            $markers = get_users_by_capability($this->evalcodeframework->get_context(), 'mod/evalcode:grade', '', $sort);
            $markerlist[0] = get_string('choosemarker', 'evalcode');
        ...
        if ($this->quickgrading && has_capability('mod/evalcode:manageallocations', $this->evalcodeframework->get_context()) &&
            (empty($row->workflowstate) ||
             $row->workflowstate == EVALCODE_MARKING_WORKFLOW_STATE_INMARKING ||
             $row->workflowstate == EVALCODE_MARKING_WORKFLOW_STATE_NOTMARKED)) {
    ...
    public function col_scale($row) {
        ...
        if (empty($this->scale)) {
            $dbparams = array('id'=>-($this->evalcodeframework->get_instance()->grade));
    ...
    public function display_grade($grade, $editable, $userid, $modified) {
        if ($this->is_downloading()) {
            if ($this->evalcodeframework->get_instance()->grade >= 0) {
                ...
                // This is a custom scale.
                $scale = $this->evalcodeframework->display_grade($grade, false);
                if ($scale == '-') {
                    $scale = '';
                }
                return $scale;
        ...
        return $this->evalcodeframework->display_grade($grade, $editable, $userid, $modified);
    }
    ...
    public function col_team(stdClass $row) {
        ...
        if ($group) {
            return $group->name;
        } else if ($this->evalcodeframework->get_instance()->preventsubmissionnotingroup) {
            $usergroups = $this->evalcodeframework->get_all_groups($row->id);
            if (count($usergroups) > 1) {
                return get_string('multipleteamsgrader', 'evalcode');
            } else {
                return get_string('noteamgrader', 'evalcode');
            }
        }
        return get_string('defaultteam', 'evalcode');
    }
    ...
    protected function get_group_and_submission($userid, &$group, &$submission, $attemptnumber) {
        ...
        if (isset($this->submissiongroups[$userid])) {
            ...
            $group = $this->evalcodeframework->get_submission_group($userid, false);
        ...
        if (isset($this->groupsubmissions[$groupid . ':' . $attemptnumber])) {
            ...
            $submission = $this->evalcodeframework->get_group_submission($userid, $groupid, false, $attemptnumber);
    ...
    public function col_fullname($row) {
        if (!$this->is_downloading()) {
            ...
            $fullname = $this->output->action_link($link, $this->evalcodeframework->fullname($row));
        } else {
            $fullname = $this->evalcodeframework->fullname($row);
        }

        if (!$this->evalcodeframework->is_active_user($row->id)) {
    ...
    public function col_select(stdClass $row) {
        ...
        $selectcol .= get_string('selectuser', 'evalcode', $this->evalcodeframework->fullname($row));
        $selectcol .= '</label>';
    ...
    /**
     * Return a users grades from the listing of all grade data for this evalcodeframework.
     *
     * @param int $userid
    ...
    public function col_grademax(stdClass $row) {
        return format_float($this->evalcodeframework->get_instance()->grade, 2);
    }
    ...
    public function col_grade(stdClass $row) {
        ...
        if (!$this->is_downloading() && $this->hasgrade) {
            $urlparams = array('id' => $this->evalcodeframework->get_course_module()->id,
                               'rownum' => 0,
                               'action' => 'grader');
    ...
    public function col_timesubmitted(stdClass $row) {
        ...
        $submission = false;
        $this->get_group_and_submission($row->id, $group, $submission, -1);
        if ($submission && $submission->timemodified && $submission->status != EVAL_SUBMISSION_STATUS_NEW) {
            $o = userdate($submission->timemodified);
        } else if ($row->timesubmitted && $row->status != EVAL_SUBMISSION_STATUS_NEW) {
    ...
    public function col_status(stdClass $row) {
        ...
        $instance = $this->evalcodeframework->get_instance();
        ...
        if ($this->evalcodeframework->is_any_submission_plugin_enabled()) {
            ...
            $o .= $this->output->container(get_string('submissionstatus_' . $displaystatus, 'evalcode'),
                                           array('class'=>'submissionstatus' .$displaystatus));
            if ($due && $timesubmitted > $due && $status != EVAL_SUBMISSION_STATUS_NEW) {
            ...
            if ($row->locked) {
                $lockedstr = get_string('submissionslockedshort', 'evalcode');
            ...
            if (!$instance->markingworkflow) {
                if ($row->grade !== null && $row->grade >= 0) {
                    $o .= $this->output->container(get_string('graded', 'evalcode'), 'submissiongraded');
                } else if (!$timesubmitted || $status == EVAL_SUBMISSION_STATUS_NEW) {
                    $now = time();
                    if ($due && ($now > $due)) {
                        $overduestr = get_string('overdue', 'evalcode', format_time($now - $due));
        ...
        if ($row->extensionduedate) {
            ...
            $extensionstr = get_string('userextensiondate', 'evalcode', $userdate);
    ...
    public function col_userid(stdClass $row) {
        ...
        if (!$row->grade) {
            ...
            $description = get_string('updategrade', 'evalcode');
        }
        ...
        $submissionsopen = $this->evalcodeframework->submissions_open($row->id,
                                                               true,
                                                               $submission,
                                                               $flags,
                                                               $this->gradinginfo);
        $caneditsubmission = $this->evalcodeframework->can_edit_submission($row->id, $USER->id);

        // Hide for offline evalcodeframeworks.
        if ($this->evalcodeframework->is_any_submission_plugin_enabled()) {
            if (!$row->status ||
                    $row->status == EVAL_SUBMISSION_STATUS_DRAFT ||
                    !$this->evalcodeframework->get_instance()->submissiondrafts) {
                ...
                if (!$row->locked) {
                    ...
                    $description = get_string('preventsubmissionsshort', 'evalcode');
                    ...
                    $urlparams = array('id' => $this->evalcodeframework->get_course_module()->id,
                                       ...
                                       'page'=>$this->currpage);
                    $url = new moodle_url('/mod/evalcode/view.php', $urlparams);
                    $description = get_string('allowsubmissionsshort', 'evalcode');
            ...
            if ($submissionsopen &&
                ...
                $urlparams = array('id' => $this->evalcodeframework->get_course_module()->id,
                                   ...
                                   'page'=>$this->currpage);
                $url = new moodle_url('/mod/evalcode/view.php', $urlparams);
                $description = get_string('editsubmission', 'evalcode');
        ...
        if (($this->evalcodeframework->get_instance()->duedate ||
                $this->evalcodeframework->get_instance()->cutoffdate) &&
             ...
             $urlparams = array('id' => $this->evalcodeframework->get_course_module()->id,
                                ...
                                'page' => $this->currpage);
             $url = new moodle_url('/mod/evalcode/view.php', $urlparams);
             $description = get_string('grantextension', 'evalcode');
        ...
        if ($row->status == EVAL_SUBMISSION_STATUS_SUBMITTED &&
                $this->evalcodeframework->get_instance()->submissiondrafts) {
            $urlparams = array('id' => $this->evalcodeframework->get_course_module()->id,
                               'userid'=>$row->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/evalcode/view.php', $urlparams);
            $description = get_string('reverttodraftshort', 'evalcode');
        ...
        if ($row->status == EVAL_SUBMISSION_STATUS_DRAFT &&
                $this->evalcodeframework->get_instance()->submissiondrafts &&
                $caneditsubmission &&
                $submissionsopen &&
                $row->id != $USER->id) {
            $urlparams = array('id' => $this->evalcodeframework->get_course_module()->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/evalcode/view.php', $urlparams);
            $description = get_string('submitforgrading', 'evalcode');
        ...
        $ismanual = $this->evalcodeframework->get_instance()->attemptreopenmethod == EVALCODE_ATTEMPT_REOPEN_METHOD_MANUAL;
        $hassubmission = !empty($row->status);
        $notreopened = $hassubmission && $row->status != EVAL_SUBMISSION_STATUS_REOPENED;
        $isunlimited = $this->evalcodeframework->get_instance()->maxattempts == EVALCODE_UNLIMITED_ATTEMPTS;
        $hasattempts = $isunlimited || $row->attemptnumber < $this->evalcodeframework->get_instance()->maxattempts - 1;
        ...
        if ($ismanual && $hassubmission && $notreopened && $hasattempts) {
            $urlparams = array('id' => $this->evalcodeframework->get_course_module()->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/evalcode/view.php', $urlparams);
            $description = get_string('addattempt', 'evalcode');
     ...
     *
     * @param evalcode_plugin $plugin Submission plugin or feedback plugin
     ...
     */
    private function format_plugin_summary_with_link(evalcode_plugin $plugin,
                                                     stdClass $item,
                                                     $returnaction,
                                                     $returnparams) {
        $link = '';
        ...
        $separator = '';
        if ($showviewlink) {
            $viewstr = get_string('view' . substr($plugin->get_subtype(), strlen('evalcode')), 'evalcode');
    ...
    public function other_cols($colname, $row) {
        ...
        if ($plugin->is_visible() && $plugin->is_enabled()) {
            if ($plugin->get_subtype() == 'evalsubmission') {
                if ($this->evalcodeframework->get_instance()->teamsubmission) {
                    $group = false;
                    $submission = false;
                    ...
                    if ($row->status == EVAL_SUBMISSION_STATUS_REOPENED) {
                        // For a newly reopened submission - we want to show the previous submission in the table.
                        $submission = $this->evalcodeframework->get_user_submission($row->userid, false, $row->attemptnumber - 1);
                    } else {
                        $submission = new stdClass();
                        $submission->id = $row->submissionid;
                        $submission->timecreated = $row->firstsubmission;
                        $submission->timemodified = $row->timesubmitted;
                        $submission->evalcodeframework = $this->evalcodeframework->get_instance()->id;
                ...
                if ($row->gradeid) {
                    ...
                    $grade->evalcodeframework = $this->evalcodeframework->get_instance()->id;
     ...
     *
     * @return string the evalcodeframework name
     */
    public function get_evalcodeframework_name() {
        return $this->evalcodeframework->get_instance()->name;
    }
    ...
    public function get_course_module_id() {
        return $this->evalcodeframework->get_course_module()->id;
    }
    ...
    public function get_course_id() {
        return $this->evalcodeframework->get_course()->id;
    }
    ...
    public function get_course_context() {
        return $this->evalcodeframework->get_course_context();
    }

    /**
     * Return things to the renderer.
     *
     * @return bool Does this evalcodeframework accept submissions
     */
    public function submissions_enabled() {
        return $this->evalcodeframework->is_any_submission_plugin_enabled();
    }
    ...
    public function can_view_all_grades() {
        $context = $this->evalcodeframework->get_course_context();
