...
/**
 * This file contains a renderer for the evalcodeframework class
 *
...
/**
 * A custom renderer class that extends the plugin_renderer_base and is used by the evalcode module.
 *
 * @package mod_evalcode
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_evalcode_renderer extends plugin_renderer_base {

    /**
     * Rendering evalcodeframework files
     *
     ...
     * @return string
     */
    public function evalcode_files(context $context, $userid, $filearea, $component) {
        return $this->render(new evalcode_files($context, $userid, $filearea, $component));
    }

    /**
     * Rendering evalcodeframework files
     *
     * @param evalcode_files $tree
     * @return string
     */
    public function render_evalcode_files(evalcode_files $tree) {
        $this->htmlid = html_writer::random_id('evalcode_files_tree');
        $this->page->requires->js_init_call('M.mod_evalcode.init_tree', array(true, $this->htmlid));
     ...
     * @param evalcode_gradingmessage $result The result to render
     * @return string
     */
    public function render_evalcode_gradingmessage(evalcode_gradingmessage $result) {
     ...
     * @param evalcode_form $form The form to render
     * @return string
     */
    public function render_evalcode_form(evalcode_form $form) {
        $o = '';
     ...
     *
     * @param evalcode_user_summary $summary The user summary to render
     * @return string
     */
    public function render_evalcode_user_summary(evalcode_user_summary $summary) {
        $o = '';
        $supendedclass = '';
        $suspendedicon = '';
        ...
        if ($summary->blindmarking) {
            $o .= get_string('hiddenuser', 'evalcode') . $summary->uniqueidforuser.$suspendedicon;
        } else {
     ...
     *
     * @param evalcode_submit_for_grading_page $page
     * @return string
     */
    public function render_evalcode_submit_for_grading_page($page) {
        $o = '';

        $o .= $this->output->container_start('submitforgrading');
        $o .= $this->output->heading(get_string('submitevalcodeframework', 'evalcode'), 3);
        $o .= $this->output->spacer(array('height'=>30));

        $cancelurl = new moodle_url('/mod/evalcode/view.php', array('id' => $page->coursemoduleid));
        if (count($page->notifications)) {
            // At least one of the submission plugins is not ready for submission.

            $o .= $this->output->heading(get_string('submissionnotready', 'evalcode'), 4);
     ...
     * Render the header.
     *
     * @param evalcode_header $header
     * @return string
     */
    public function render_evalcode_header(evalcode_header $header) {
        $o = '';
        ...
        $this->page->set_title(get_string('pluginname', 'evalcode'));
        $this->page->set_heading($this->page->course->fullname);

        $o .= $this->output->header();
        $heading = format_string($header->evalcode->name, false, array('context' => $header->context));
        ...
        if ($header->showintro) {
            ...
            $o .= format_module_intro('evalcode', $header->evalcode, $header->coursemoduleid);
            $o .= $header->postfix;
            $o .= $this->output->box_end();
        }
     ...
     *
     * @param evalcode_plugin_header $header
     * @return string
     */
    public function render_evalcode_plugin_header(evalcode_plugin_header $header) {
     ...
     *
     * @param evalcode_grading_summary $summary
     * @return string
     */
    public function render_evalcode_grading_summary(evalcode_grading_summary $summary) {
        // Create a table for the data.
        $o = '';
        $o .= $this->output->container_start('gradingsummary');
        $o .= $this->output->heading(get_string('gradingsummary', 'evalcode'), 3);
        ...
        if ($summary->teamsubmission) {
            if ($summary->warnofungroupedusers) {
                $o .= $this->output->notification(get_string('ungroupedusers', 'evalcode'));
            }

            $this->add_table_row_tuple($t, get_string('numberofteams', 'evalcode'),
                                       $summary->participantcount);
        } else {
            $this->add_table_row_tuple($t, get_string('numberofparticipants', 'evalcode'),
                                       $summary->participantcount);
        }

        // Drafts count and dont show drafts count when using offline evalcodeframework.
        if ($summary->submissiondraftsenabled && $summary->submissionsenabled) {
            $this->add_table_row_tuple($t, get_string('numberofdraftsubmissions', 'evalcode'),
                                       $summary->submissiondraftscount);
        }

        // Submitted for grading.
        if ($summary->submissionsenabled) {
            $this->add_table_row_tuple($t, get_string('numberofsubmittedevalcodeframeworks', 'evalcode'),
                                       $summary->submissionssubmittedcount);
            if (!$summary->teamsubmission) {
                $this->add_table_row_tuple($t, get_string('numberofsubmissionsneedgrading', 'evalcode'),
        ...
        if ($summary->duedate) {
            ...
            $this->add_table_row_tuple($t, get_string('duedate', 'evalcode'),
                                       userdate($duedate));
            ...
            $this->add_table_row_tuple($t, get_string('timeremaining', 'evalcode'), $due);

            if ($duedate < $time) {
                $cutoffdate = $summary->cutoffdate;
                if ($cutoffdate) {
                    if ($cutoffdate > $time) {
                        $late = get_string('latesubmissionsaccepted', 'evalcode', userdate($summary->cutoffdate));
                    } else {
                        $late = get_string('nomoresubmissionsaccepted', 'evalcode');
                    }
                    $this->add_table_row_tuple($t, get_string('latesubmissions', 'evalcode'), $late);
                }
            }
        ...
        $url = new moodle_url('/mod/evalcode/view.php', $urlparams);
        $o .= '<a href="' . $url . '" class="btn">' . get_string('viewgrading', 'mod_evalcode') . '</a> ';
     ...
     *
     * @param eval_feedback_status $status
     * @return string
     */
    public function render_eval_feedback_status(eval_feedback_status $status) {
        global $DB, $CFG;
        $o = '';

        $o .= $this->output->container_start('feedback');
        $o .= $this->output->heading(get_string('feedback', 'evalcode'), 3);
        ...
        if (isset($status->gradefordisplay)) {
            ...
            // Grade date.
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('gradedon', 'evalcode'));
        ...
        if ($status->grader) {
            // Grader.
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('gradedby', 'evalcode'));
        ...
        foreach ($status->feedbackplugins as $plugin) {
            if ($plugin->is_enabled() &&
                ...
                $displaymode = eval_feedback_plugin_feedback::SUMMARY;
                $pluginfeedback = new eval_feedback_plugin_feedback($plugin,
                                                                      $status->grade,
                                                                      $displaymode,
     ...
     *
     * @param eval_submission_status_compact $status
     * @return string
     */
    public function render_eval_submission_status_compact(eval_submission_status_compact $status) {
        $o = '';
        $o .= $this->output->container_start('submissionstatustable');
        $o .= $this->output->heading(get_string('submission', 'evalcode'), 3);
        $time = time();
        ...
        if ($status->teamsubmissionenabled) {
            ...
            if ($group) {
                ...
                if (count($status->usergroups) == 0) {
                    $team = '<span class="alert alert-error">' . get_string('noteam', 'evalcode') . '</span>';
                } else if (count($status->usergroups) > 1) {
                    $team = '<span class="alert alert-error">' . get_string('multipleteams', 'evalcode') . '</span>';
                }
            } else {
                $team = get_string('defaultteam', 'evalcode');
            }
            $o .= $this->output->container(get_string('teamname', 'evalcode', $team), 'teamname');
        }

        if (!$status->teamsubmissionenabled) {
            if ($status->submission && $status->submission->status != EVAL_SUBMISSION_STATUS_NEW) {
                $statusstr = get_string('submissionstatus_' . $status->submission->status, 'evalcode');
                $o .= $this->output->container($statusstr, 'submissionstatus' . $status->submission->status);
            } else {
                if (!$status->submissionsenabled) {
                    $o .= $this->output->container(get_string('noonlinesubmissions', 'evalcode'), 'submissionstatus');
                } else {
                    $o .= $this->output->container(get_string('noattempt', 'evalcode'), 'submissionstatus');
                }
            }
        } else {
            $group = $status->submissiongroup;
            if (!$group && $status->preventsubmissionnotingroup) {
                $o .= $this->output->container(get_string('nosubmission', 'evalcode'), 'submissionstatus');
            } else if ($status->teamsubmission && $status->teamsubmission->status != EVAL_SUBMISSION_STATUS_NEW) {
                $teamstatus = $status->teamsubmission->status;
                $submissionsummary = get_string('submissionstatus_' . $teamstatus, 'evalcode');
                $groupid = 0;
                ...
                foreach ($members as $member) {
                    ...
                    $url = new moodle_url('/user/view.php', $urlparams);
                    if ($status->view == eval_submission_status::GRADER_VIEW && $status->blindmarking) {
                ...
                if (count($userslist) > 0) {
                    ...
                    $formatteduserstr = get_string('userswhoneedtosubmit', 'evalcode', $userstr);
        ...
        // Is locked?
        if ($status->locked) {
            $o .= $this->output->container(get_string('submissionslocked', 'evalcode'), 'submissionlocked');
        }

        // Grading status.
        $statusstr = '';
        $classname = 'gradingstatus';
        if ($status->gradingstatus == EVALCODE_GRADING_STATUS_GRADED ||
            $status->gradingstatus == EVALCODE_GRADING_STATUS_NOT_GRADED) {
            $statusstr = get_string($status->gradingstatus, 'evalcode');
        } else {
            $gradingstatus = 'markingworkflowstate' . $status->gradingstatus;
            $statusstr = get_string($gradingstatus, 'evalcode');
        }
        if ($status->gradingstatus == EVALCODE_GRADING_STATUS_GRADED ||
            $status->gradingstatus == EVALCODE_MARKING_WORKFLOW_STATE_RELEASED) {
        ...
        if ($duedate > 0) {
            ...
            if ($duedate - $time <= 0) {
                if (!$submission ||
                    ...
                    if ($status->submissionsenabled) {
                        $remaining = get_string('overdue', 'evalcode', format_time($time - $duedate));
                        $classname = 'overdue';
                    } else {
                        $remaining = get_string('duedatereached', 'evalcode');
                    }
                } else {
                ...
                $remaining = get_string('paramtimeremaining', 'evalcode', format_time($duedate - $time));
            }
            $o .= $this->output->container($remaining, $classname);
        }

        // Show graders whether this submission is editable by students.
        if ($status->view == eval_submission_status::GRADER_VIEW) {
            if ($status->canedit) {
                $o .= $this->output->container(get_string('submissioneditable', 'evalcode'), 'submissioneditable');
            } else {
                $o .= $this->output->container(get_string('submissionnoteditable', 'evalcode'), 'submissionnoteditable');
            }
        }
            ...
            if (!$status->teamsubmission || $status->submissiongroup != false || !$status->preventsubmissionnotingroup) {
                foreach ($status->submissionplugins as $plugin) {
                    ...
                    if ($plugin->is_enabled() &&
                        ...
                        $displaymode = eval_submission_plugin_submission::SUMMARY;
                        $pluginsubmission = new eval_submission_plugin_submission($plugin,
                            $submission,
                            $displaymode,
                            $status->coursemoduleid,
                            $status->returnaction,
                        ...
                        $o .= $this->output->container($this->render($pluginsubmission), 'evalsubmission ' . $plugincomponent);
                    }
                }
            }
        }
     ...
     *
     * @param eval_submission_status $status
     * @return string
     */
    public function render_eval_submission_status(eval_submission_status $status) {
        $o = '';
        $o .= $this->output->container_start('submissionstatustable');
        $o .= $this->output->heading(get_string('submissionstatusheading', 'evalcode'), 3);
        $time = time();
//print_r(""<br><br>start<br><br>");
        if ($status->allowsubmissionsfromdate &&
            ...
            if ($status->alwaysshowdescription) {
                ...
                $o .= get_string('allowsubmissionsfromdatesummary', 'evalcode', $date);
            } else {
                $date = userdate($status->allowsubmissionsfromdate);
                $o .= get_string('allowsubmissionsanddescriptionfromdatesummary', 'evalcode', $date);
            }
            $o .= $this->output->box_end();
        }
        ...
        if ($status->teamsubmissionenabled) {
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('submissionteam', 'evalcode'));
            $group = $status->submissiongroup;
            if ($group) {
                ...
                if (count($status->usergroups) == 0) {
                    $cell2 = new html_table_cell(
                        html_writer::span(get_string('noteam', 'evalcode'), 'alert alert-error')
                    );
                } else if (count($status->usergroups) > 1) {
                    $cell2 = new html_table_cell(
                        html_writer::span(get_string('multipleteams', 'evalcode'), 'alert alert-error')
                    );
                }
            } else {
                $cell2 = new html_table_cell(get_string('defaultteam', 'evalcode'));
            }
            $row->cells = array($cell1, $cell2);
            $t->data[] = $row;
        }

        if ($status->attemptreopenmethod != EVALCODE_ATTEMPT_REOPEN_METHOD_NONE) {
            $currentattempt = 1;
            if (!$status->teamsubmissionenabled) {
            ...
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('attemptnumber', 'evalcode'));
            $maxattempts = $status->maxattempts;
            if ($maxattempts == EVALCODE_UNLIMITED_ATTEMPTS) {
                $message = get_string('currentattempt', 'evalcode', $currentattempt);
            } else {
                $message = get_string('currentattemptof', 'evalcode', array('attemptnumber'=>$currentattempt,
        ...
        $row = new html_table_row();
        $cell1 = new html_table_cell(get_string('submissionstatus', 'evalcode'));
        if (!$status->teamsubmissionenabled) {
            if ($status->submission && $status->submission->status != EVAL_SUBMISSION_STATUS_NEW) {
                $statusstr = get_string('submissionstatus_' . $status->submission->status, 'evalcode');
                ...
                if (!$status->submissionsenabled) {
                    $cell2 = new html_table_cell(get_string('noonlinesubmissions', 'evalcode'));
                } else {
                    $cell2 = new html_table_cell(get_string('noattempt', 'evalcode'));
                }
            }
            $row->cells = array($cell1, $cell2);
            $t->data[] = $row;
            ...
            $group = $status->submissiongroup;
            if (!$group && $status->preventsubmissionnotingroup) {
                $cell2 = new html_table_cell(get_string('nosubmission', 'evalcode'));
            } else if ($status->teamsubmission && $status->teamsubmission->status != EVAL_SUBMISSION_STATUS_NEW) {
                $teamstatus = $status->teamsubmission->status;
                $submissionsummary = get_string('submissionstatus_' . $teamstatus, 'evalcode');
                $groupid = 0;
        ...
        if ($status->locked) {
            ...
            $cell2 = new html_table_cell(get_string('submissionslocked', 'evalcode'));
        ...
        // Grading status.
        $row = new html_table_row();
        $cell1 = new html_table_cell(get_string('gradingstatus', 'evalcode'));

        if ($status->gradingstatus == EVALCODE_GRADING_STATUS_GRADED ||
            $status->gradingstatus == EVALCODE_GRADING_STATUS_NOT_GRADED) {
            $cell2 = new html_table_cell(get_string($status->gradingstatus, 'evalcode'));
        } else {
            $gradingstatus = 'markingworkflowstate' . $status->gradingstatus;
            $cell2 = new html_table_cell(get_string($gradingstatus, 'evalcode'));
        }
        if ($status->gradingstatus == EVALCODE_GRADING_STATUS_GRADED ||
            $status->gradingstatus == EVALCODE_MARKING_WORKFLOW_STATE_RELEASED) {
        ...
        if ($duedate > 0) {
            // Due date.
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('duedate', 'evalcode'));
            ...
            if ($status->view == eval_submission_status::GRADER_VIEW) {
                if ($status->cutoffdate) {
                    // Cut off date.
                    $row = new html_table_row();
                    $cell1 = new html_table_cell(get_string('cutoffdate', 'evalcode'));
            ...
            if ($status->extensionduedate) {
                // Extension date.
                $row = new html_table_row();
                $cell1 = new html_table_cell(get_string('extensionduedate', 'evalcode'));
            ...
            // Time remaining.
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('timeremaining', 'evalcode'));
            if ($duedate - $time <= 0) {
                if (!$submission ||
                        $submission->status != EVAL_SUBMISSION_STATUS_SUBMITTED) {
                    if ($status->submissionsenabled) {
                        $overduestr = get_string('overdue', 'evalcode', format_time($time - $duedate));
                        $cell2 = new html_table_cell($overduestr);
                        $cell2->attributes = array('class'=>'overdue');
                    } else {
                        $cell2 = new html_table_cell(get_string('duedatereached', 'evalcode'));
                    }
                } else {
                    if ($submission->timemodified > $duedate) {
        ...
        if ($status->view == eval_submission_status::GRADER_VIEW) {
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('editingstatus', 'evalcode'));
            if ($status->canedit) {
                $cell2 = new html_table_cell(get_string('submissioneditable', 'evalcode'));
                $cell2->attributes = array('class'=>'submissioneditable');
            } else {
                $cell2 = new html_table_cell(get_string('submissionnoteditable', 'evalcode'));
        ...
        if (!empty($status->gradingcontrollerpreview)) {
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('gradingmethodpreview', 'evalcode'));
        ...
        if ($submission) {
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('timemodified', 'evalcode'));
        ...
        $o .= $this->output->box_end();
        // Links.
        if ($status->view == eval_submission_status::STUDENT_VIEW) {

            if ($status->canedit) {
                if (!$submission || $submission->status == EVAL_SUBMISSION_STATUS_NEW) {
                    $o .= $this->output->box_start('generalbox submissionaction');
                    $urlparams = array('id' => $status->coursemoduleid, 'action' => 'editsubmission');
                    $o .= $this->output->single_button(new moodle_url('/mod/evalcode/view.php', $urlparams),
                                                       get_string('addsubmission', 'evalcode'), 'get');
                    $o .= $this->output->box_start('boxaligncenter submithelp');
                    $o .= get_string('editsubmission_help', 'evalcode');
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_end();
                } else if ($submission->status == EVAL_SUBMISSION_STATUS_REOPENED) {
                    $o .= $this->output->box_start('generalbox submissionaction');
                    $urlparams = array('id' => $status->coursemoduleid,
                                       'action' => 'editprevioussubmission',
                                       'sesskey'=>sesskey());
                    $o .= $this->output->single_button(new moodle_url('/mod/evalcode/view.php', $urlparams),
                                                       get_string('addnewattemptfromprevious', 'evalcode'), 'get');
                    $o .= $this->output->box_start('boxaligncenter submithelp');
                    $o .= get_string('addnewattemptfromprevious_help', 'evalcode');
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_start('generalbox submissionaction');
                    $urlparams = array('id' => $status->coursemoduleid, 'action' => 'editsubmission');
                    $o .= $this->output->single_button(new moodle_url('/mod/evalcode/view.php', $urlparams),
                                                       get_string('addnewattempt', 'evalcode'), 'get');
                    $o .= $this->output->box_start('boxaligncenter submithelp');
                    $o .= get_string('addnewattempt_help', 'evalcode');
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_end();
                } else {
                    $o .= $this->output->box_start('generalbox submissionaction');
                    $urlparams = array('id' => $status->coursemoduleid, 'action' => 'editsubmission');
                    $o .= $this->output->single_button(new moodle_url('/mod/evalcode/view.php', $urlparams),
                                                       get_string('editsubmission', 'evalcode'), 'get');
                    $o .= $this->output->box_start('boxaligncenter submithelp');
                    $o .= get_string('editsubmission_help', 'evalcode');
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_end();
            ...
            if ($status->cansubmit) {
                ...
                $o .= $this->output->box_start('generalbox submissionaction');
                $o .= $this->output->single_button(new moodle_url('/mod/evalcode/view.php', $urlparams),
                                                   get_string('submitevalcodeframework', 'evalcode'), 'get');
                $o .= $this->output->box_start('boxaligncenter submithelp');
                $o .= get_string('submitevalcodeframework_help', 'evalcode');
                $o .= $this->output->box_end();
                $o .= $this->output->box_end();
            }
        }
    ...
    /**
     * Output the attempt history chooser for this evalcodeframework
     *
     * @param evalcode_attempt_history_chooser $history
     * @return string
     */
    public function render_evalcode_attempt_history_chooser(evalcode_attempt_history_chooser $history) {
        $o = '';

        $context = $history->export_for_template($this);
        $o .= $this->render_from_template('mod_evalcode/attempt_history_chooser', $context);

        return $o;
    }

    /**
     * Output the attempt history for this evalcodeframework
     *
     * @param evalcode_attempt_history $history
     * @return string
     */
    public function render_evalcode_attempt_history(evalcode_attempt_history $history) {
        $o = '';

        $submittedstr = get_string('submitted', 'evalcode');
        $gradestr = get_string('grade');
        $gradedonstr = get_string('gradedon', 'evalcode');
        $gradedbystr = get_string('gradedby', 'evalcode');
        ...
        $o .= $this->output->heading(get_string('attempthistory', 'evalcode'), 3);
        ...
        foreach ($history->submissions as $i => $submission) {
            ...
            if ($submission) {
                ...
                $submissionsummary = get_string('nosubmission', 'evalcode');
            }

            $attemptsummaryparams = array('attemptnumber'=>$submission->attemptnumber+1,
                                          'submissionsummary'=>$submissionsummary);
            $o .= $this->heading(get_string('attemptheading', 'evalcode', $attemptsummaryparams), 4);

            $t = new html_table();

            if ($submission) {
                $cell1 = new html_table_cell(get_string('submissionstatus', 'evalcode'));
                $cell2 = new html_table_cell(get_string('submissionstatus_' . $submission->status, 'evalcode'));
                ...
                foreach ($history->submissionplugins as $plugin) {
                    ...
                    if ($plugin->is_enabled() &&
                        ...
                        $pluginsubmission = new eval_submission_plugin_submission($plugin,
                                                                                    $submission,
                                                                                    eval_submission_plugin_submission::SUMMARY,
            ...
            if ($grade) {
                // Heading 'feedback'.
                $title = get_string('feedback', 'evalcode', $i);
                ...
                if ($history->cangrade) {
                    ...
                    $icon = new pix_icon('gradefeedback',
                                            get_string('editattemptfeedback', 'evalcode', $grade->attemptnumber+1),
                                            'mod_evalcode');
                ...
                foreach ($history->feedbackplugins as $plugin) {
                    if ($plugin->is_enabled() &&
                        ...
                        $cell1 = new html_table_cell($plugin->get_name());
                        $pluginfeedback = new eval_feedback_plugin_feedback(
                            $plugin, $grade, eval_feedback_plugin_feedback::SUMMARY, $history->coursemoduleid,
        ...
        $this->page->requires->yui_module('moodle-mod_evalcode-history', 'Y.one("#' . $containerid . '").history');
     ...
     *
     * @param eval_submission_plugin_submission $submissionplugin
     * @return string
     */
    public function render_eval_submission_plugin_submission(eval_submission_plugin_submission $submissionplugin) {
        $o = '';

        if ($submissionplugin->view == eval_submission_plugin_submission::SUMMARY) {
            $showviewlink = false;
            ...
            $link = '';
            if ($showviewlink) {
                $previewstr = get_string('viewsubmission', 'evalcode');
                $icon = $this->output->pix_icon('t/preview', $previewstr);

                $expandstr = get_string('viewfull', 'evalcode');
                ...
                $this->page->requires->js_init_call('M.mod_evalcode.init_plugin_summary', $jsparams);
            ...
            if ($showviewlink) {
                ...
                $o .= $this->output->pix_icon('t/switch_minus',
                                              get_string('viewsummary', 'evalcode'),
                                              null,
                                              array('class'=>$classes));
                $o .= $submissionplugin->plugin->view($submissionplugin->submission);
                $o .= $this->output->box_end();
            }
        } else if ($submissionplugin->view == eval_submission_plugin_submission::FULL) {
     ...
     *
     * @param evalcode_grading_table $table
     * @return string
     */
    public function render_evalcode_grading_table(evalcode_grading_table $table) {
        $o = '';
        $o .= $this->output->box_start('boxaligncenter gradingtable');

        $this->page->requires->js_init_call('M.mod_evalcode.init_grading_table', array());
        $this->page->requires->string_for_js('nousersselected', 'evalcode');
        $this->page->requires->string_for_js('batchoperationconfirmgrantextension', 'evalcode');
        $this->page->requires->string_for_js('batchoperationconfirmlock', 'evalcode');
        $this->page->requires->string_for_js('batchoperationconfirmreverttodraft', 'evalcode');
        ...
        $this->page->requires->string_for_js('editaction', 'evalcode');
     ...
     *
     * @param eval_feedback_plugin_feedback $feedbackplugin
     * @return string
     */
    public function render_eval_feedback_plugin_feedback(eval_feedback_plugin_feedback $feedbackplugin) {
        $o = '';

        if ($feedbackplugin->view == eval_feedback_plugin_feedback::SUMMARY) {
            $showviewlink = false;
            ...
            $link = '';
            if ($showviewlink) {
                $previewstr = get_string('viewfeedback', 'evalcode');
                $icon = $this->output->pix_icon('t/preview', $previewstr);
        ...
        } else if ($feedbackplugin->view == eval_feedback_plugin_feedback::FULL) {
     ...
     *
     * @param evalcode_course_index_summary $indexsummary
     * @return string
     */
    public function render_evalcode_course_index_summary(evalcode_course_index_summary $indexsummary) {
        $o = '';
        ...
        $strsubmission = get_string('submission', 'evalcode');
        ...
        $currentsection = '';
        foreach ($indexsummary->evalcodeframeworks as $info) {
            $params = array('id' => $info['cmid']);
            $link = html_writer::link(new moodle_url('/mod/evalcode/view.php', $params),
                                      $info['cmname']);
     ...
     * @return string
     */
    protected function htmllize_tree(evalcode_files $tree, $dir) {
        global $CFG;
        $yuiconfig = array();
        $yuiconfig['type'] = 'html';
     ...
     * @param int $rowsperpage How many evalcodeframeworks to render in a page
    ...
    public function render_grading_app(grading_app $app) {
        ...
        return $this->render_from_template('mod_evalcode/grading_app', $context);
    }
}
