 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_evalcode_grade_form extends moodleform {
    /** @var evalcodeframework $evalcodeframework */
    private $evalcodeframework;
    ...
    public function definition() {
        ...
        list($evalcodeframework, $data, $params) = $this->_customdata;
        // Visible elements.
        $this->evalcodeframework = $evalcodeframework;
        $evalcodeframework->add_grade_form_elements($mform, $data, $params);
    ...
    public function validation($data, $files) {
        ...
        if ($instance->markingworkflow && !empty($data['sendstudentnotifications']) &&
                $data['workflowstate'] != EVALCODE_MARKING_WORKFLOW_STATE_RELEASED) {
        ...
        if ($instance->grade > 0) {
            if (unformat_float($data['grade'], true) === false && (!empty($data['grade']))) {
                $errors['grade'] = get_string('invalidfloatforgrade', 'evalcode', $data['grade']);
            } else if (unformat_float($data['grade']) > $instance->grade) {
                $errors['grade'] = get_string('gradeabovemaximum', 'evalcode', $instance->grade);
            } else if (unformat_float($data['grade']) < 0) {
                $errors['grade'] = get_string('gradebelowzero', 'evalcode');
            }
        } else {
            // This is a scale.
            if ($scale = $DB->get_record('scale', array('id'=>-($instance->grade)))) {
                ...
                if ((int)$data['grade'] !== -1 && !array_key_exists((int)$data['grade'], $scaleoptions)) {
                    $errors['grade'] = get_string('invalidgradeforscale', 'evalcode');
                }
            }
        }
        return $errors;
