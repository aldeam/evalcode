...
/**
 * This file contains the classes for the admin settings of the evalcode module.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class evalcode_admin_page_manage_evalcode_plugins extends admin_externalpage {
    ...
    public function __construct($subtype) {
        ...
        $url = new moodle_url('/mod/evalcode/adminmanageplugins.php', array('subtype'=>$subtype));
        parent::__construct('manage' . $subtype . 'plugins',
                            get_string('manage' . $subtype . 'plugins', 'evalcode'),
                            $url);
    }
...
class evalcode_plugin_manager {
    ...
    /**
     * Constructor for this evalcodeframework plugin manager
     * @param string $subtype - either evalsubmission or evalfeedback
     */
    public function __construct($subtype) {
        $this->pageurl = new moodle_url('/mod/evalcode/adminmanageplugins.php', array('subtype'=>$subtype));
        $this->subtype = $subtype;
    }
    ...
    private function view_plugins_table() {
        ...
        $table->define_columns(array('pluginname', 'version', 'hideshow', 'order',
                'settings', 'uninstall'));
        $table->define_headers(array(get_string($this->subtype . 'pluginname', 'evalcode'),
                get_string('version'), get_string('hideshow', 'evalcode'),
        ...
        if (($this->subtype == "evalfeedback")||($this->subtype == "evalsubmission")) {
            $shortsubtype = substr($this->subtype, strlen('eval'));
        } else {
            $shortsubtype = substr($this->subtype, strlen('evalcode'));
        }
        ...
        foreach ($plugins as $idx => $plugin) {
            ...
            $exists = file_exists($CFG->dirroot . '/mod/evalcode/' . $shortsubtype . '/' . $plugin . '/settings.php');
            if ($row[1] != '' && $exists) {
    ...
    private function view_header() {
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('manage' . $this->subtype . 'plugins', 'evalcode'));
    }
