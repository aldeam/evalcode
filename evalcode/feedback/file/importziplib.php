 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class evalfeedback_file_zip_importer {

    /**
     * Is this filename valid (contains a unique participant ID) for import?
     *
     * @param evalcode $evalcodeframework - The evalcodeframework instance
     ...
     * @param evalcode_plugin $plugin - Set to the plugin that exported the file
     ...
     */
    public function is_valid_filename_for_import($evalcodeframework, $fileinfo, $participants, & $user, & $plugin, & $filename) {
        ...
        $filename = $info[4];
        $plugin = $evalcodeframework->get_plugin_by_type($info[2], $info[3]);
     ...
     * @param evalcode_plugin $plugin The matching plugin from the filename
     ...
     */
    public function is_file_modified($evalcodeframework, $user, $plugin, $filename, $fileinfo) {
        $sg = null;

        if ($plugin->get_subtype() == 'evalsubmission') {
            $sg = $evalcodeframework->get_user_submission($user->id, false);
        } else if ($plugin->get_subtype() == 'evalfeedback') {
            $sg = $evalcodeframework->get_user_grade($user->id, false);
        } else {
            return false;
        }
     ...
     *
     * @param int $contextid - The context id of this evalcodeframework instance
     ...
     * @param int $contextid The context for this evalcodeframework
    ...
    public function extract_files_from_zip($zipfile, $contextid) {
        ...
        core_php_time_limit::raise(EVALFEEDBACK_FILE_MAXFILEUNZIPTIME);
     ...
     *
     * @param evalcode $evalcodeframework - The evalcodeframework instance
     * @param eval_feedback_file $fileplugin - The file feedback plugin
     * @return string - The html response
     */
    public function import_zip_files($evalcodeframework, $fileplugin) {
        global $CFG, $PAGE, $DB;
        ...
        foreach ($files as $unzippedfile) {
            ...
            if ($this->is_valid_filename_for_import($evalcodeframework, $unzippedfile, $participants, $user, $plugin, $filename)) {
                if ($this->is_file_modified($evalcodeframework, $user, $plugin, $filename, $unzippedfile)) {
                    $grade = $evalcodeframework->get_user_grade($user->id, true);
                    ...
                    if ($oldfile = $fs->get_file($contextid,
                        ...
                        $newfilerecord->component = 'evalfeedback_file';
                        $newfilerecord->filearea = EVALFEEDBACK_FILE_FILEAREA;
                    ...
                    $evalcodeframework->notify_grade_modified($grade);
                }
            }
        }

        require_once($CFG->dirroot . '/mod/evalcode/feedback/file/renderable.php');
        $importsummary = new evalfeedback_file_import_summary($evalcodeframework->get_course_module()->id,
                                                            count($userswithnewfeedback),
                                                            $feedbackfilesadded,
                                                            $feedbackfilesupdated);

        $evalcoderenderer = $evalcodeframework->get_renderer();
        $renderer = $PAGE->get_renderer('evalfeedback_file');

        $o = '';

        $o .= $evalcoderenderer->render(new evalcode_header($evalcodeframework->get_instance(),
                                                        $evalcodeframework->get_context(),
                                                        false,
                                                        $evalcodeframework->get_course_module()->id,
                                                        get_string('uploadzipsummary', 'evalfeedback_file')));

        $o .= $renderer->render($importsummary);

        $o .= $evalcoderenderer->render_footer();
        return $o;
    }
