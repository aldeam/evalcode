...
// File areas for file feedback evalcodeframework.
define('EVALFEEDBACK_FILE_FILEAREA', 'feedback_files');
define('EVALFEEDBACK_FILE_BATCH_FILEAREA', 'feedback_files_batch');
define('EVALFEEDBACK_FILE_IMPORT_FILEAREA', 'feedback_files_import');
define('EVALFEEDBACK_FILE_MAXSUMMARYFILES', 5);
define('EVALFEEDBACK_FILE_MAXSUMMARYUSERS', 5);
define('EVALFEEDBACK_FILE_MAXFILEUNZIPTIME', 120);
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class eval_feedback_file extends eval_feedback_plugin
{
    ...
    {
        return get_string('file', 'evalfeedback_file');
    }
    ...
    {
        global $DB;
        return $DB->get_record('evalfeedback_file', array('grade' => $gradeid));
    }
    ...
    public function is_feedback_modified(stdClass $grade, stdClass $data)
    {
        ...
        if (isset($filekey)) {
            ...
            $filecount = $this->count_files($grade->id, EVALFEEDBACK_FILE_FILEAREA);
     ...
     * @return boolean
     */
    public function copy_area_files(file_storage $fs,
                                     $fromcontextid,
                                     $fromcomponent,
                                     $fromfilearea,
                                     $fromitemid,
     ...
     * @return int
     */
    public function count_files($gradeid, $area)
    {
    ...
    public function update_file_count($grade)
    {
        ...
        if ($filefeedback) {
            $filefeedback->numfiles = $this->count_files($grade->id, EVALFEEDBACK_FILE_FILEAREA);
            $a = $DB->update_record('evalfeedback_file', $filefeedback);
            return $a;
        } else {
            $filefeedback = new stdClass();
            $filefeedback->numfiles = $this->count_files($grade->id, EVALFEEDBACK_FILE_FILEAREA);
            $filefeedback->grade = $grade->id;
            $filefeedback->evalcodeframework = $this->evalcodeframework->get_instance()->id;
            $a = $DB->insert_record('evalfeedback_file', $filefeedback) > 0;
            return $a;
    ...
    public function view_summary(stdClass $grade, & $showviewlink)
    {
        ...
        $count = $this->count_files($grade->id, EVALFEEDBACK_FILE_FILEAREA);

        // Show a view all link if the number of files is over this limit.
        $showviewlink = $count > EVALFEEDBACK_FILE_MAXSUMMARYFILES;

        if ($count <= EVALFEEDBACK_FILE_MAXSUMMARYFILES) {
            return $this->evalcodeframework->render_area_files('evalfeedback_file',
                EVALFEEDBACK_FILE_FILEAREA,
                $grade->id);
        } else {
            return get_string('countfiles', 'evalfeedback_file', $count);
        }
    }
    ...
    public function delete_instance()
    {
        ...
        $DB->delete_records('evalfeedback_file',
    ...
    {
        return $this->count_files($grade->id, EVALFEEDBACK_FILE_FILEAREA) == 0;
    }
    ...
    {
        return array(EVALFEEDBACK_FILE_FILEAREA => $this->get_name());
    }
     ...
     * and version.
     *
     * @param string $type old evalcodeframework subtype
     * @param int $version old evalcodeframework version
    ...
    /**
     * Upgrade the settings from the old evalcodeframework to the new plugin based one.
     *
     * @param context $oldcontext - the context for the old evalcodeframework
     * @param stdClass $oldevalcodeframework - the data for the old evalcodeframework
    ...
    /**
     * Upgrade the feedback from the old evalcodeframework to the new one.
     *
     * @param context $oldcontext - the database for the old evalcodeframework context
     * @param stdClass $oldevalcodeframework The data record for the old evalcodeframework
                            ...
                            & $log)
    {
        ...
        $filefeedback->grade = $grade->id;
        $filefeedback->evalcodeframework = $this->evalcodeframework->get_instance()->id;
        if (!$DB->insert_record('evalfeedback_file', $filefeedback) > 0) {
            $log .= get_string('couldnotconvertgrade', 'mod_evalcode', $grade->userid);
            return false;
        }
        return true;
    }
    ...
    {
        return array('uploadfiles' => get_string('uploadfiles', 'evalfeedback_file'));
    }
    ...
    public function view_batch_upload_files($users)
    {
        ...
        require_capability('mod/evalcode:grade', $this->evalcodeframework->get_context());
        require_once($CFG->dirroot . '/mod/evalcode/feedback/file/batchuploadfilesform.php');
        require_once($CFG->dirroot . '/mod/evalcode/renderable.php');

        $formparams = array('cm' => $this->evalcodeframework->get_course_module()->id,
            'users' => $users,
            'context' => $this->evalcodeframework->get_context());

        $usershtml = '';

        $usercount = 0;
        foreach ($users as $userid) {
            if ($usercount >= EVALFEEDBACK_FILE_MAXSUMMARYUSERS) {
                $moreuserscount = count($users) - EVALFEEDBACK_FILE_MAXSUMMARYUSERS;
                $usershtml .= get_string('moreusers', 'evalfeedback_file', $moreuserscount);
                break;
            }
            $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);

            $usersummary = new evalcode_user_summary($user,
                $this->evalcodeframework->get_course()->id,
                has_capability('moodle/site:viewfullnames',
                    $this->evalcodeframework->get_course_context()),
                $this->evalcodeframework->is_blind_marking(),
                $this->evalcodeframework->get_uniqueid_for_user($user->id),
                get_extra_user_fields($this->evalcodeframework->get_context()));
            $usershtml .= $this->evalcodeframework->get_renderer()->render($usersummary);
            $usercount += 1;
        }

        $formparams['usershtml'] = $usershtml;

        $mform = new evalfeedback_file_batch_upload_files_form(null, $formparams);

        if ($mform->is_cancelled()) {
            redirect(new moodle_url('view.php',
                array('id' => $this->evalcodeframework->get_course_module()->id,
                    'action' => 'grading')));
            return;
            ...
            $data = file_postupdate_standard_filemanager($data,
                ...
                $this->evalcodeframework->get_context(),
                'evalfeedback_file',
                EVALFEEDBACK_FILE_BATCH_FILEAREA,
                $USER->id);
            $fs = get_file_storage();

            // Now copy each of these files to the users feedback file area.
            foreach ($users as $userid) {
                $grade = $this->evalcodeframework->get_user_grade($userid, true);
                $this->evalcodeframework->notify_grade_modified($grade);
                ...
                if ($filefeedback) {
                    $filefeedback->numfiles = $this->count_files($grade->id,
                        EVALFEEDBACK_FILE_FILEAREA);
                    $DB->update_record('evalfeedback_file', $filefeedback);
                } else {
                    $filefeedback = new stdClass();
                    $filefeedback->numfiles = $this->count_files($grade->id,
                        EVALFEEDBACK_FILE_FILEAREA);
                    $filefeedback->grade = $grade->id;
                    $filefeedback->evalcodeframework = $this->evalcodeframework->get_instance()->id;
                    $DB->insert_record('evalfeedback_file', $filefeedback);
                }
            }

            // Now delete the temporary import area.
            $fs->delete_area_files($this->evalcodeframework->get_context()->id,
                'evalfeedback_file',
                EVALFEEDBACK_FILE_BATCH_FILEAREA,
                $USER->id);
            ...
            $header = new evalcode_header($this->evalcodeframework->get_instance(),
                $this->evalcodeframework->get_context(),
                false,
                $this->evalcodeframework->get_course_module()->id,
                get_string('batchuploadfiles', 'evalfeedback_file'));
            $o = '';
            $o .= $this->evalcodeframework->get_renderer()->render($header);
            $o .= $this->evalcodeframework->get_renderer()->render(new evalcode_form('batchuploadfiles', $mform));
            $o .= $this->evalcodeframework->get_renderer()->render_footer();
        }
    ...
    public function view_upload_zip()
    {
        ...
        require_capability('mod/evalcode:grade', $this->evalcodeframework->get_context());
        require_once($CFG->dirroot . '/mod/evalcode/feedback/file/uploadzipform.php');
        require_once($CFG->dirroot . '/mod/evalcode/feedback/file/importziplib.php');
        require_once($CFG->dirroot . '/mod/evalcode/feedback/file/importzipform.php');

        $formparams = array('context' => $this->evalcodeframework->get_context(),
            'cm' => $this->evalcodeframework->get_course_module()->id);
        $mform = new evalfeedback_file_upload_zip_form(null, $formparams);

        $o = '';

        $confirm = optional_param('confirm', 0, PARAM_BOOL);
        $renderer = $this->evalcodeframework->get_renderer();

        // Delete any existing files.
        $importer = new evalfeedback_file_zip_importer();
        ...
        if ($mform->is_cancelled()) {
            ...
            $params = array('evalcodeframework' => $this->evalcodeframework, 'importer' => $importer);

            $mform = new evalfeedback_file_import_zip_form(null, $params);
            if ($mform->is_cancelled()) {
                $importer->delete_import_files($contextid);
                $urlparams = array('id' => $this->evalcodeframework->get_course_module()->id,
                    'action' => 'grading');
            ...
            $o .= $importer->import_zip_files($this->evalcodeframework, $this);
            $importer->delete_import_files($contextid);
        } else if (($data = $mform->get_data()) &&
            ($zipfile = $mform->save_stored_file('feedbackzip',
                $contextid,
                'evalfeedback_file',
                EVALFEEDBACK_FILE_IMPORT_FILEAREA,
                $USER->id,
                '/',
                'import.zip',
                true))
            ...
            $header = new evalcode_header($this->evalcodeframework->get_instance(),
                ...
                false,
                $this->evalcodeframework->get_course_module()->id,
                get_string('confirmuploadzip', 'evalfeedback_file'));
            $o .= $renderer->render($header);
            $o .= $renderer->render(new evalcode_form('confirmimportzip', $mform));
            ...
            $header = new evalcode_header($this->evalcodeframework->get_instance(),
                ...
                false,
                $this->evalcodeframework->get_course_module()->id,
                get_string('uploadzip', 'evalfeedback_file'));
            $o .= $renderer->render($header);
            $o .= $renderer->render(new evalcode_form('uploadfeedbackzip', $mform));
            $o .= $renderer->render_footer();
        }
    ...
    /**
     * Called by the evalcodeframework module when someone chooses something from the
    ...
    {
        return array('uploadzip' => get_string('uploadzip', 'evalfeedback_file'));
    }
