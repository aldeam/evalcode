 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class eval_feedback_comments extends eval_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'evalfeedback_comments');
    }
    ...
    public function get_feedback_comments($gradeid) {
        global $DB;
        return $DB->get_record('evalfeedback_comments', array('grade'=>$gradeid));
    }
    ...
    public function get_quickgrading_html($userid, $grade) {
        ...
        $pluginname = get_string('pluginname', 'evalfeedback_comments');
    ...
    public function is_feedback_modified(stdClass $grade, stdClass $data) {
        ...
        if ($commenttext == $data->evalfeedbackcomments_editor['text']) {
            return false;
        } else {
            return true;
        }
    ...
    public function get_editor_fields() {
        return array('comments' => get_string('pluginname', 'evalfeedback_comments'));
    }
    ...
    public function set_editor_text($name, $value, $gradeid) {
        ...
        if ($name == 'comments') {
            ...
            if ($feedbackcomment) {
                ...
                return $DB->update_record('evalfeedback_comments', $feedbackcomment);
            } else {
                $feedbackcomment = new stdClass();
                $feedbackcomment->commenttext = $value;
                $feedbackcomment->commentformat = FORMAT_HTML;
                $feedbackcomment->grade = $gradeid;
                $feedbackcomment->evalcodeframework = $this->evalcodeframework->get_instance()->id;
                return $DB->insert_record('evalfeedback_comments', $feedbackcomment) > 0;
            }
        }
    ...
    public function save_settings(stdClass $data) {
        $this->set_config('commentinline', !empty($data->evalfeedback_comments_commentinline));
        return true;
    }
    ...
    public function get_settings(MoodleQuickForm $mform) {
        ...
        if ($default === false) {
            ...
            $default = get_config('evalfeedback_comments', 'inline');
        }
        $mform->addElement('selectyesno',
                           'evalfeedback_comments_commentinline',
                           get_string('commentinline', 'evalfeedback_comments'));
        $mform->addHelpButton('evalfeedback_comments_commentinline', 'commentinline', 'evalfeedback_comments');
        $mform->setDefault('evalfeedback_comments_commentinline', $default);
        // Disable comment online if comment feedback plugin is disabled.
        $mform->disabledIf('evalfeedback_comments_commentinline', 'evalfeedback_comments_enabled', 'notchecked');
   }
    ...
    protected function convert_submission_text_to_feedback($submission, $data) {
        ...
        $data->evalfeedbackcomments_editor['text'] = $text;
        $data->evalfeedbackcomments_editor['format'] = $format;
    ...
    public function get_form_elements_for_user($grade, MoodleQuickForm $mform, stdClass $data, $userid) {
        ...
        $submission = $this->evalcodeframework->get_user_submission($userid, false);
        ...
        if ($feedbackcomments && !empty($feedbackcomments->commenttext)) {
            $data->evalfeedbackcomments_editor['text'] = $feedbackcomments->commenttext;
            $data->evalfeedbackcomments_editor['format'] = $feedbackcomments->commentformat;
        } else {
        ...
        $mform->addElement('editor', 'evalfeedbackcomments_editor', $this->get_name(), null, null);
    ...
    public function save(stdClass $grade, stdClass $data) {
        ...
        $feedbackcomment = $this->get_feedback_comments($grade->id);
        if ($feedbackcomment) {
            $feedbackcomment->commenttext = $data->evalfeedbackcomments_editor['text'];
            $feedbackcomment->commentformat = $data->evalfeedbackcomments_editor['format'];
            return $DB->update_record('evalfeedback_comments', $feedbackcomment);
        } else {
            $feedbackcomment = new stdClass();
            $feedbackcomment->commenttext = $data->evalfeedbackcomments_editor['text'];
    ...
    public function view_summary(stdClass $grade, & $showviewlink) {
        ...
        if ($feedbackcomments) {
            $text = format_text($feedbackcomments->commenttext,
                                ...
                                array('context' => $this->evalcodeframework->get_context()));
    ...
    public function upgrade_settings(context $oldcontext, stdClass $oldevalcodeframework, & $log) {
        if ($oldevalcodeframework->evalcodeframeworktype == 'online') {
            $this->set_config('commentinline', $oldevalcodeframework->var1);
            return true;
        }
        return true;
    }
    ...
    public function upgrade(context $oldcontext,
        ...
        $feedbackcomments->evalcodeframework = $this->evalcodeframework->get_instance()->id;
        if (!$DB->insert_record('evalfeedback_comments', $feedbackcomments) > 0) {
     ...
     * of the comment
     *
     * Only one feedback plugin can push comments to the gradebook and that is chosen by the evalcodeframework
     * settings page.
     *
     * @param stdClass $grade The grade
     * @return int
    ...
    public function delete_instance() {
        ...
        $DB->delete_records('evalfeedback_comments',
    ...
    public function get_external_parameters() {
        ...
        return array('evalfeedbackcomments_editor' => $editorstructure);
    }
